...
const BulkOpperationsControls: FC<Props> = ({
  bulkCount,
  bulkList,
  handleEditMode,
  selectMode,
  ...
  const { recipeItems, bulkUpdateRecipes, setRecipeItems } =
  ...
  const handleConfirmed = () => {
    ...
    if (user?.userId) {
      switch (actionType) {
        case "favorite":
          favoritesService
            .addFavoriteBulk(bulkList, user.userId, token)
            .then((resp: IFavoritesBulkResponse) => {
              setRecipeItems(bulkUpdateRecipes(resp.data, recipeItems));
              addToast(`Success: ${confirmMsg}`, {
                appearance: "success",
                autoDismiss: true,
              });
            })
        ...
        case "unfavorite":
          favoritesService
            .removeFavoriteBulk(bulkList, user.userId, token)
            .then((resp: IFavoritesBulkResponse) => {
              setRecipeItems(bulkUpdateRecipes(resp.data, recipeItems));
              addToast(`Success: ${confirmMsg}`, {
                appearance: "success",
        ...
        case "share":
          recipeService
            .addShareBulk(bulkList, user.userId, token)
            .then((resp: IShareResponse) => {
              setRecipeItems(bulkUpdateRecipes(resp.data, recipeItems));
              addToast(`Success: ${confirmMsg}`, {
                appearance: "success",
                autoDismiss: true,
        ...
        case "noshare":
          recipeService
            .removeShareBulk(bulkList, user.userId, token)
            .then((resp: IShareResponse) => {
              setRecipeItems(bulkUpdateRecipes(resp.data, recipeItems));
              addToast(`Success: ${confirmMsg}`, {
                appearance: "success",
  ...
  return (
    <StyledControlWrap>
        ...
        <React.Fragment>
          ...
          <StyledToolBarDivider>
            <StyledMoreIconButton
              ...
              title={`${bulkCount} item${bulkCount !== 1 ? "s" : ""} selected`}
            >
              <StyledMoreIcon />
              <span>({bulkCount})</span>
            </StyledMoreIconButton>
      ...
      )}
      {
        <StyledToolBarDivider>
          <StyledToggleButton
            ...
            title={selectMode ? "Cancel select items" : "Select items"}
          >
      ...
      <ConfirmDialog
        ...
        title={"Are you sure?"}
        confirmText={`${
          actionItemEnums[actionType as keyof typeof actionItemEnums]
        } for ${bulkCount} ${bulkCount !== 1 ? "items" : "item"}`}
